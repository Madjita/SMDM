...
///
/// \defgroup ResManagerFunc Функции менеджера ресурсов
///@{

/////////////////////////////////////////////////////////////////////////////
/// Извлечение идентификатора менеджера ресурсов
/// \param[out] vi Указатель на идентификатор менеджера ресурсов
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_SYSTEM_ERROR
///
...
/////////////////////////////////////////////////////////////////////////////
/// Создание новой сессии подключения в текущем менеджере ресурсов. Идентификатор менеджера должен быть извлечен функцией viOpenDefaultRM.
/// \param[in] sesn Идентификатор менеджера ресурсов
/// \param[in] name Адрес подключения
/// \param[in] mode Определяет режим, в котором осуществляется подключение(не реализованно)
/// \param[in] timeout Таймаут, по которому осуществляется блокировка соединения, если того требует режим доступа(не реализованно)
/// \param[out] vi Указатель на идентификатор создаваемой сессии подключения
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_INV_OBJECT
/// \li \ref VI_ERROR_INV_RSRC_NAME
/// \li \ref VI_ERROR_RSRC_NFOUND
/// \li \ref VI_ERROR_TMO
///
...
/// \param[in] vi Идентификатор менеджера ресурсов или сессии соединения. Если передан идентификатор сессии, то она будет закрыта. если передан идентификатор менеджера ресурсов, будут закрыты все сессии, открытые в этом менеджере.
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_INV_OBJECT
/// \li \ref VI_ERROR_CLOSING_FAILED
///
...
///
/// \defgroup SesnAttrFunc Функции управления атрибутами сессии
///@{

/////////////////////////////////////////////////////////////////////////////
/// Установка атрибутов сессии подключения
/// \param[in] vi Идентификатор сессии соединения. 
/// \param[in] attrName Имя атрибута, значение которого будет менятся(в данный момент поддерживается только установка таймаута с именем атрибута VI_ATTR_TMO_VALUE).
/// \param[in] attrValue Новое значение атрибута.
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_INV_OBJECT
/// \li \ref VI_ERROR_NSUP_ATTR
///

ViStatus _VI_FUNC  viSetAttribute  (ViObject vi, ViAttr attrName, ViAttrState attrValue);

/////////////////////////////////////////////////////////////////////////////
/// Чтение атрибутов сессии подключения
/// \param[in] vi Идентификатор сессии соединения. 
/// \param[in] attrName Имя атрибута, значение которого будет считываться(в данный момент поддерживается только установка таймаута с именем атрибута VI_ATTR_TMO_VALUE).
/// \param[out] attrValue Считанное значение атрибута.
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_INV_OBJECT
/// \li \ref VI_ERROR_NSUP_ATTR
...
///
/// \defgroup IOFunc Функции чтения/записи
///@{

/////////////////////////////////////////////////////////////////////////////
/// Финкция чтения данных. 
/// \param[in] vi Идентификатор сессии соединения. 
/// \param[out] buf Указатель на буфер, в который производится чтение.
/// \param[in] cnt Количество байт, которое необходимо прочитать.
/// \param[out] retCnt Количество байт, которое было считано.
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_INV_OBJECT
/// \li \ref VI_ERROR_IO
...
/////////////////////////////////////////////////////////////////////////////
/// Финкция записи данных. 
/// \param[in] vi Идентификатор сессии соединения. 
/// \param[out] buf Буфер с данными.
/// \param[in] cnt Количество байт, которое необходимо записать.
/// \param[out] retCnt Количество байт, которое было записано.
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_INV_OBJECT
/// \li \ref VI_ERROR_IO
...
/////////////////////////////////////////////////////////////////////////////
/// Финкция восстановления соединения. 
/// \param[in] vi Идентификатор сессии соединения. 
/// \return код ошибки
/// \li \ref VI_SUCCESS 
/// \li \ref VI_ERROR_INV_OBJECT
/// \li \ref VI_ERROR_CONN_LOST
/// \li \ref VI_ERROR_TMO
///
...
ViStatus _VI_FUNC  viUsbControlOut   (ViSession vi, ViInt16 bmRequestType, ViInt16 bRequest,
                                      ViUInt16 wValue, ViUInt16 wIndex, ViUInt16 wLength, ViBuf buf);
...
#define VI_ATTR_TMO_VALUE           (0x3FFF001AUL) ///< Таймаут. Устанавливает одинаковый таймаут на соединение, чтение и запись.
...
#define VI_ATTR_BREAK_PACKET		(0x3FFF0246UL)
#define VI_ATTR_FULL_HEAD			(0x3FFF0247UL)
#define VI_ATTR_BODY				(0x3FFF0248UL)
...
/// 
/// \defgroup ErrorCodes Коды возвращаемых ошибок
///@{
...
#define VI_ERROR_SYSTEM_ERROR       (_VI_ERROR+0x3FFF0000L) /* BFFF0000, -1073807360 */ ///< Ошибка инициализации библиотеки VISA.
#define VI_ERROR_INV_OBJECT         (_VI_ERROR+0x3FFF000EL) /* BFFF000E, -1073807346 */ ///< Неправильный идентификатор сессии(объекта). Возможно передан нулевой идентификатор сессии.
...
#define VI_ERROR_RSRC_NFOUND        (_VI_ERROR+0x3FFF0011L) /* BFFF0011, -1073807343 */	///< Ресурс не найден. Ошибка соответсвует ошибкам TCPIP сокетов WSAECONNREFUSED.
#define VI_ERROR_INV_RSRC_NAME      (_VI_ERROR+0x3FFF0012L) /* BFFF0012, -1073807342 */ ///< Неверный формат адреса ресурса. Требуемый формат адреса для протокола TCPIP выглядит следующим образом: "TCPIP::адрес устройства::порт::SOCKET". 
#define VI_ERROR_INV_ACC_MODE       (_VI_ERROR+0x3FFF0013L) /* BFFF0013, -1073807341 */
#define VI_ERROR_TMO                (_VI_ERROR+0x3FFF0015L) /* BFFF0015, -1073807339 */ ///< Таймаут выполнения операции. Ошибка соответсвует ошибкам TCPIP сокетов WSAETIMEDOUT и при подключении WSAEHOSTUNREACH.
#define VI_ERROR_CLOSING_FAILED     (_VI_ERROR+0x3FFF0016L) /* BFFF0016, -1073807338 */ ///< Ошибка при закрытии сессии.
...
#define VI_ERROR_NSUP_ATTR          (_VI_ERROR+0x3FFF001DL) /* BFFF001D, -1073807331 */ ///< Неподдерживаемый тип атрибута. На данный момент поддерживается только один атрибут: \ref VI_ATTR_TMO_VALUE.
...
#define VI_ERROR_IO                 (_VI_ERROR+0x3FFF003EL) /* BFFF003E, -1073807298 */ ///< Ошибка ввода/вывода при чтении или записи.
...
#define VI_ERROR_NIMPL_OPER         (_VI_ERROR+0x3FFF0081L) /* BFFF0081, -1073807231 */ ///< Операция не реализована.
...
#define VI_ERROR_LIBRARY_NFOUND     (_VI_ERROR+0x3FFF009EL) /* BFFF009E, -1073807202 */ ///< Не найдена библиотека необходимая для работы сессии
...
#define VI_ERROR_CONN_LOST          (_VI_ERROR+0x3FFF00A6L) /* BFFF00A6, -1073807194 */ ///< Соединение с прибором потерянно и попытка восстановить его обернулась неудачей. Ошибка соответсвует ошибке TCPIP сокетов WSAECONNRESET.
